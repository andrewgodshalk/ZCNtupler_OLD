...
VHbbCandidate HbbCandidateFinderAlgo::changeHiggs(bool useHighestPtHiggs , const VHbbCandidate & old)
{
  ...
  if (useHighestPtHiggs == false)
  {
    ...
    findDiJets(jets,j1,j2,addJets,temp.H.indices) ;
  }
  else
  {
    //foundJets= findDiJetsHighestPt(jets,j1,j2,addJets,temp.H.indices) ;
    findDiJetsHighestPt(jets,j1,j2,addJets,temp.H.indices) ;
  }
...
// run() - Takes input VHbbEvent, VHbbEventAuxInfo from Step1 Ntuple, spits out vector of VHbbCandidates
void HbbCandidateFinderAlgo::run (const VHbbEvent* event, std::vector<VHbbCandidate> & candidates, const VHbbEventAuxInfo & aux )
{
  ...
  // Find valid electons, muons, taus
  ...
  // DISREGARDING THESE RESULTS. In fact, not planning on storing the higgs information.
  VHbbEvent::SimpleJet j1,j2;
  std::vector<VHbbEvent::SimpleJet> addJets;
  bool foundJets;
  if (useHighestPtHiggs_ == false) foundJets = findDiJets(	    noOverlap,j1,j2,addJets,temp.H.indices);
  else 				   foundJets = findDiJetsHighestPt( noOverlap,j1,j2,addJets,temp.H.indices);
  //if (useHighestPtHiggs_ == false) foundJets = findDiJets(	      noOverlap,j1,j2,addJets);
  //else 			     foundJets = findDiJetsHighestPt( noOverlap,j1,j2,addJets);

  if (verbose_) std::cout <<" Found Dijets: "<<foundJets<< " Additional: "<<addJets.size()<< std::endl;  
  ...
  if (verbose_) std::cout <<" Electrons: " << ele.size() << std::endl
			  <<" Muons    : " << mu.size()  << std::endl
			  <<" Taus     : " << tau.size() << std::endl	
			  <<" MET      : " << met.size() << std::endl;

  // CHANGED
  //if (ele.size()<1 && mu.size() < 1 && met.size()<1 && tau.size()<1) return;
  if (ele.size()<1 && mu.size() < 1 ) return;

  // Store Higgs information in appropriate object
  ...
  // now see which kind of candidate this can be
  VHbbCandidate result;
  bool ok = false;

  // first: hZmumu
  
  if (verbose_) std::cout <<" START SELECTION "<<std::endl;
...
{
  if (verbose_) std::cout <<" CandidateFinder: Input Jets = "<<jetsin.size()<<std::endl;\
  ...
  if (jets.size()>2)
    for (unsigned int i=0 ; i< jets.size(); ++i)
      if (i != index1 && i != index2 ) addJets.push_back(jets[i]);
  ...
  */
  ...
  for (unsigned int it = 0; it < muons.size(); ++it)
  {
    ...
    */
	...
	// && muons[it].nHits > 10
	// && (muons[it].cat & 0x2)	// tracker
	 && (muons[it].cat & 0x1)	// global
	 && muons[it].nMatches >= 2
	 && muons[it].ipDb     <  0.2
	 && muons[it].zPVPt    <  0.5
	// && (muons[it].hIso+muons[it].eIso+muons[it].tIso)/muons[it].p4.Pt()<.15
	 && pfCorrIso 		     < 0.12
	 && fabs(muons[it].p4.Eta()) < 2.4
	 && muons[it].p4.Pt()	     > 20			// PT CUT ON MUONS. MADE IN THIS FUNCTION.
      )
    {
  ...
  if (verbose_) std::cout <<" CandidateFinder: Input Muons = "<<muons.size()<<" Output Muons = "<<out.size()<<std::endl;
}
  ...
  */
  ...
  for (unsigned int  it = 0; it< electrons.size(); ++it)
  {
    ...
    */
    ...
    if(electrons[it].innerHits>0) pho-=electrons[it].pfPhoIsoDoubleCounted;
    ...
    if (verbose_) std::cout <<" CandidateFinder: Input Electrons = "<<electrons.size()<<" Output Electrons = "<<out.size()<<std::endl;
