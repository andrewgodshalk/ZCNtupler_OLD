...
class VHbbCandidateTools {
 ...
 VHbbCandidateTools(bool verbose = false): verbose_(verbose){}
  ...
  {
    if (verbose_) std::cout <<" getHZmumuCandidate input mu "<<in.V.muons.size()<<" e "<<in.V.electrons.size()<<std::endl;
    ok = false;
    VHbbCandidate temp = in;
    ...
    // If leading muon pt < 20, return input.
    if (muons_[0].p4.Pt()<20 || muons_[1].p4.Pt()<20 ) return in;

    // Find highest pt pair w/ opposite sign.
    int selectMu2=1;
    if (muons_[0].charge*muons_[selectMu2].charge > 0) // if same sign,
    {
	...
	if(muons_[it].charge * muons_[0].charge < 0)
	{
      ...
      if (selectMu2 == 1) return in;	// If no pair of opp sign found, break.
    }

    // In temp, combine 4-momentum of muon pair
    temp.V.p4 = muons_[0].p4 + muons_[selectMu2].p4;

    // Find muons w/ pt>20, add to temp 		(REDUNDANT? Should already be pT>20 on way in.)
    ...
    // Find electrons from original w/ pT>20, put back in temp
  ...
  VHbbCandidate getHZeeCandidate(const VHbbCandidate & in, bool & ok, std::vector<unsigned int>& pos){
	...
	if (  electrons_[it].charge * electrons_[0].charge < 0) {
      ...
      if (selectE2 == 1) return in;				// CHANGED: Swapped from row above. Now outside for loop, should return 
    }
