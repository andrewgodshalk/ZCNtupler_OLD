  ...
  public:
	...
	class VectorCandidate {
	  public:
		...
		// Mt() - Calculates mass of possible W using leading lepton + met
   ...
   public:
	...
	// Helper functions - public access to nested class data
	double deltaPhi() const {return V.p4.DeltaPhi(H.p4);}				// Return DelPhi btwn V & H Candidates
	double Mt() 		const {return V.Mt(candidateType);}				// Return VB mass (calculated if W, 0 if Z)
	double MtTau() 	const {return V.MtTau(candidateTypeWithTau);}	// " w/ Tau
	
	// additionalLeptons() - Return the number of additional leptons left after accounting for leptons in either W (1) or Z (2)
